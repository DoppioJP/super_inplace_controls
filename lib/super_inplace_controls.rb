...
module Flvorful
  module SuperInplaceControls
    ...
    module HelperMethods
      ...
      # Creates an "active" text field control with auto complete (auto_complete plugin required) 
      # that submits any changes to the server
      # using an in_place_edit-style action.
      # You still need to send :completion_options within options[:completion_options] as you normaly send them
      # in text_field_with_auto_complete
      # By default the value of the object's attribute will be filled in or blank.
      # Example:
      #   <%= in_place_text_field_with_auto_complete :product, :title %>
      def in_place_text_field_with_auto_complete(object, method, options = {})
        in_place_field(:text_field_with_auto_complete, object, method, options)
      end
      ...
      def in_place_field(field_type, object, method, options)
        ...
        @object = options[:object] || self.instance_variable_get("@#{object}")
      ...
      def html_for_inplace_display(object_name, method_name, object, display_text)
        ...
        :onclick => update_page do |page|
				    page.hide "#{id_string}"
						page.show "#{id_string }_form"
						page.select("##{id_string }_form input").last.focus
				  end, 
      ...
      def form_for_inplace_display(object_name, method_name, input_type, object, opts)
        ...
        retval << form_remote_tag(:url => { :controller => object.class.name.tableize, :action => set_method, :id => object.id}.merge(opts[:url_params] || {}),
        ...
        retval << hidden_field_tag("_method", :put)
        retval << hidden_field_tag("#{object_name}[format]", opts[:format]) if opts[:format]
        opts[:onblur] = update_page do |page|
					page.show "#{id_string}"
					page.hide "#{id_string}_form"
				end
        retval << field_for_inplace_editing(object_name, method_name, object, opts, input_type )
        retval << content_tag(:br) if opts[:br]
        retval << submit_tag( save_button_text, :class => "inplace_submit") unless opts[:hide_submit_button]
        retval << link_to_function( "Cancel", update_page do |page|
					page.show "#{id_string}"
					page.hide "#{id_string}_form"
				end, {:class => "inplace_cancel" }) unless opts[:hide_cancel_link]
      ...
      def field_for_inplace_editing(object_name, method_name, object, options, input_type)
        ...
        # Completion options needed only for text_field_with_auto_complete
        completion_options = options[:completion_options]
        options.delete(:completion_options)
        
        # Pass only input options and delete keys not needed
        input_options = {}.merge(options)
        
        # Delete not needed keys
        keys_not_needed = [:action, :auto_complete, :hide_cancel_link, :hide_submit_button, :saving_text, :url_params]
        keys_not_needed.map { |key| input_options.delete(key) }
        
        
        case input_type
        when :text_field
          text_field(object_name, method_name, input_options)
        when :text_area
          text_area(object_name, method_name, input_options)
        when :select
          select(object_name, method_name,  options[:choices], input_options, htm_opts )
        when :check_box
          options[:label_class] = "inplace_#{input_type}_label"
          checkbox_collection(object_name, method_name, object,  options[:choices], input_options )
        when :radio
          options[:label_class] = "inplace_#{input_type}_label"
          radio_collection(object_name, method_name, object,  options[:choices], input_options )
        when :date_select
          calendar_date_select( object_name, method_name, input_options)
        when :text_field_with_auto_complete
          #TODO: make sure auto_complete plugin is there

          input_options.merge!({ :id => id_string_for(object_name, method_name, object) + "_text_field" })
          
          text_field_with_auto_complete object_name, method_name, input_options, completion_options
        end
      end
